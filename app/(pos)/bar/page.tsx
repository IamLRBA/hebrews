...
export default function BarLoginPage() {
    ...
    posFetch('/api/bar/shifts')
      ...
      .then((data) => {
        if (data.shifts && data.shifts.length > 0) {
          ...
          router.replace(`/bar/${data.shifts[0].id}`)
        } else {
          setAvailableShifts([])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null)
    try {
      router.push(`/bar/${shiftId.trim()}`)
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to load bar display')
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="pos-page-container w-full max-w-md px-4">
          <div className="pos-card shadow-2xl">
            <div className="text-center mb-8">
              ...
              <h1 className="pos-section-title text-3xl mb-2">Bar Display</h1>
