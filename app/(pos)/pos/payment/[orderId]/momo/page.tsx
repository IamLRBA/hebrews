...
import { getOrderDetailOfflineFormatted, payMomoOffline } from '@/lib/offline/offline-order-service'
...
const MTN_MERCHANT_CODE = '99203376'
const MTN_DIAL = '*165*3#'
...
export default function PosPaymentMomoPage() {
  ...
  async function handleCompletePayment() {
    ...
    try {
      if (!isOnline()) {
        await payMomoOffline({ orderLocalId: orderId, amountUgx: order.totalUgx })
  ...
  return (
    <main className="pos-page min-h-screen flex flex-col">
      <div className="flex-shrink-0 fixed top-0 left-0 right-0 z-[1020] bg-[var(--color-bg-primary)] px-4 pt-4">
        <PosNavHeader />
      </div>
      ...
      <div className="flex-1 flex flex-col items-center p-6 overflow-auto pt-6">
        <div className="pos-card w-full max-w-lg p-8 text-center">
          <h1 className="pos-section-title text-2xl mb-6 mt-4">Pay using MTN MoMo</h1>
          ...
          <div className="space-y-5 text-center mb-8 p-6 bg-neutral-50 dark:bg-neutral-800/50 rounded-xl border border-neutral-200 dark:border-neutral-700">
            <div>
              <p className="m-0 text-sm text-neutral-500 dark:text-neutral-400">Merchant Code:</p>
              <p className="m-0 mt-1 font-mono font-semibold text-neutral-900 dark:text-neutral-100">{MTN_MERCHANT_CODE}</p>
            </div>
            <div>
              <p className="m-0 text-sm text-neutral-500 dark:text-neutral-400">Dial:</p>
              <p className="m-0 mt-1 font-mono text-lg font-semibold text-neutral-900 dark:text-neutral-100">{MTN_DIAL}</p>
            </div>
