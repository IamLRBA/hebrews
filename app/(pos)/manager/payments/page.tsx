              ...
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <div className="relative">
                  <select
                    ...
                    onChange={(e) => { setStatusFilter(e.target.value); setStatusDropdownOpen(false) }}
                ...
                <div className="relative">
                  <select
                    ...
                    onChange={(e) => { setMethodFilter(e.target.value); setMethodDropdownOpen(false) }}
                    className="pos-input w-full md:w-48 pr-10 appearance-none"
                  >
                ...
                <div className="pos-data-table-wrap border-2 border-neutral-100 dark:border-neutral-800">
                  <table className="w-full">
                    <thead className="bg-white dark:bg-neutral-800">
                      <tr>
                        <th className="text-left py-3 px-6 text-sm font-semibold text-neutral-700 dark:text-neutral-300 rounded-tl-lg">Order #</th>
                        <th className="text-left py-3 px-6 text-sm font-semibold text-neutral-700 dark:text-neutral-300">Amount</th>
                        <th className="text-left py-3 px-6 text-sm font-semibold text-neutral-700 dark:text-neutral-300">Method</th>
                        <th className="text-left py-3 px-6 text-sm font-semibold text-neutral-700 dark:text-neutral-300">Status</th>
                        <th className="text-left py-3 px-6 text-sm font-semibold text-neutral-700 dark:text-neutral-300">Staff</th>
                        <th className="text-left py-3 px-6 text-sm font-semibold text-neutral-700 dark:text-neutral-300 rounded-tr-lg">Date</th>
                      </tr>
                    </thead>
                    <tbody>
                      {payments
                        .filter((p) => !searchQuery || p.orderNumber.toLowerCase().includes(searchQuery.toLowerCase()))
                        .map((payment, idx) => (
                          <Fragment key={payment.id}>
                            ...
                            <tr className="border-t border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                              <td className="py-4 px-6 text-sm font-medium text-neutral-900 dark:text-neutral-100">{payment.orderNumber}</td>
                              <td className="py-4 px-6 text-sm text-neutral-900 dark:text-neutral-100">{payment.amountUgx.toLocaleString()} UGX</td>
                              <td className="py-4 px-6 text-sm text-neutral-600 dark:text-neutral-400 capitalize">{payment.method.replace('_', ' ')}</td>
                              ...
                              <td className="py-4 px-6 text-sm text-neutral-600 dark:text-neutral-400">{payment.createdByStaffName}</td>
                              <td className="py-4 px-6 text-sm text-neutral-600 dark:text-neutral-400">{new Date(payment.createdAt).toLocaleString()}</td>
                            </tr>
                          </Fragment>
                        ))}
                    </tbody>
