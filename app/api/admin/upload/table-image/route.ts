...
export async function POST(request: NextRequest) {
  try {
    ...
    const filename = `table-${crypto.randomUUID()}${safeExt}`
    const dir = path.join(process.cwd(), 'public', 'pos-images', 'tables')
    await mkdir(dir, { recursive: true })
    const filePath = path.join(dir, filename)
    const buffer = Buffer.from(await file.arrayBuffer())
    await writeFile(filePath, buffer)

    const publicPath = `/pos-images/tables/${filename}`
    return NextResponse.json({ path: publicPath })
  } catch (error) {
    console.error('[admin/upload/table-image] POST error:', error)
