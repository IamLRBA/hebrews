...
const NAV_ITEMS = [
  { href: '/kitchen', label: 'Pending', iconOutline: IconClock, iconFilled: IconClockFilled, requiresShiftId: false },
  { href: '/kitchen/preparing', label: 'Preparing', iconOutline: IconChefHat, iconFilled: IconChefHatFilled, requiresShiftId: true },
] as const

export function KitchenNavHeader({ shiftId }: { shiftId?: string }) {
    ...
    <header className="pos-dashboard-header flex flex-col gap-4 mb-6 pos-dashboard-header-sticky">
      <div className="flex flex-row items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 ml-auto">
          <span className="text-sm text-neutral-500 dark:text-neutral-400 font-medium">Kitchen Display</span>
          <SettingsDropdown />
        </div>
      </div>
      <nav className="pos-dashboard-nav" aria-label="Kitchen sections">
        {NAV_ITEMS.map(({ href, label, iconOutline, iconFilled, requiresShiftId }) => {
          ...
          const finalHref = requiresShiftId && shiftId ? `${href}?shiftId=${shiftId}` : href
          return (
            <Link
              key={href}
              href={finalHref}
