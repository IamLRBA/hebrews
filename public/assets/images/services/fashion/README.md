# Fashion Service Images

## Folder Structure

Place your actual product images in the following folders:

### Service Thumbnails (4 images per service)

- `shirts/thumb1.jpg` - Gentle
- `shirts/thumb2.jpg` - Checked  
- `shirts/thumb3.jpg` - Textured
- `shirts/thumb4.jpg` - Denim

- `tees/thumb1.jpg` - Plain
- `tees/thumb2.jpg` - Graphic
- `tees/thumb3.jpg` - Collared
- `tees/thumb4.jpg` - Sporty

- `coats/thumb1.jpg` - Sweater
- `coats/thumb2.jpg` - Hoodie
- `coats/thumb3.jpg` - Coat
- `coats/thumb4.jpg` - Jacket

- `pants-and-shorts/thumb1.jpg` - Gentle
- `pants-and-shorts/thumb2.jpg` - Denim
- `pants-and-shorts/thumb3.jpg` - Cargo
- `pants-and-shorts/thumb4.jpg` - Sporty

- `footwear/thumb1.jpg` - Gentle
- `footwear/thumb2.jpg` - Sneakers
- `footwear/thumb3.jpg` - Sandals
- `footwear/thumb4.jpg` - Boots

- `accessories/thumb1.jpg` - Rings & Necklaces
- `accessories/thumb2.jpg` - Shades & Glasses
- `accessories/thumb3.jpg` - Bracelets & Watches
- `accessories/thumb4.jpg` - Decor

### Image Specifications

- **Format**: JPG, PNG, or WebP
- **Recommended Size**: 800x800px or larger (square aspect ratio)
- **File Size**: Under 500KB for optimal performance
- **Quality**: High quality, well-lit product images

### Current Status

Currently using SVG placeholder images. These will automatically be replaced when you add your JPG images with the same filenames.

### Main Service Images

Also update these main service banner images in the parent directory:

- `shirts.jpg` - Main banner for Shirts category
- `tees.jpg` - Main banner for Tees category  
- `coats.jpg` - Main banner for Coats category
- `pants-and-shorts.jpg` - Main banner for Pants & Shorts category
- `footwear.jpg` - Main banner for Footwear category
- `accessories.jpg` - Main banner for Accessories category

---

**Note**: SVG placeholders are currently in use. Simply replace them with your JPG images using the exact filenames listed above.

